...
- [ ] **Convex service is ready** — a hosted Convex project exists, and Cloud Agents have credentials (login or deploy key in secrets) to use that service. Local `convex dev` alone does **not** satisfy this. The final project must include and use the Convex service. A Convex account does **not** exist yet.
