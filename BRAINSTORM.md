...
| Disconnect | **20 second grace** to reconnect (same `sessionId`). Match waits. If one is still gone when grace ends: **that leaver loses**, opponent advances; leaver splash is **you lost**. If **both** are still gone: Convex picks a **random player in the pair to advance**; the other is eliminated (**you lost**). |
